...
`MISSKEY_KEYWORDS_FILE`で指定したファイルでは、1行を1つの検索条件として扱います。

- 行同士はOR条件です。いずれかの行に一致すれば対象になります。
- 同じ行に半角スペースで複数の語を書くとAND条件になります。すべての語を含むNoteだけが一致します。
- 空行と各行の前後の空白は無視されます。
- 1行にキーワードを1つだけ書く従来の形式もそのまま利用できます。
- `#news`のようなハッシュタグも指定できます。

例えば、`keywords.txt`を次のように記述します。
...
この場合、「アーモンドアイ」を含むNote、または「アイ」と「ウマ娘」の両方を含むNoteが一致します。「アイ」だけ、あるいは「ウマ娘」だけを含むNoteは2行目の条件には一致しません。
